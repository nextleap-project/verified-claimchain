...
val tlcountered: #a: eqtype -> #f:cmp(a) -> sl: skipList a f -> counter : nat{ counter < length sl} -> Tot(skipList a f)(decreases(counter))
...
value : a -> comparator:(a -> a -> Tot int)-> Tot(bool)(decreases (length sl))

let rec search_ #a #f sl value comparator = 
    let values = getValues sl in 
        if Seq.length values <= 0 then false 
        else 
            let indexes = getIndex sl 0 in 
            let result = temp_func comparator values indexes value in 
            match result with 
            |Found r -> r 
            |NotFound r -> 
                    match (tl_countered (Some sl) r) with 
                    |None -> false
                    |Some sl -> search_ #a #f sl value comparator 
(*)
