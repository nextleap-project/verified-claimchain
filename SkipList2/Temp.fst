...
val lemma_temp: #a : eqtype -> #f: cmp a -> place: nat{place > 0}-> 
                    s: seq a {Seq.sorted f s /\ Seq.length s > (place +1)} -> 
                    Lemma
                        (requires
                            (Seq.sorted f (fst (Seq.split s place))))
                        (ensures 
...
(*)
val lemma_temp: #a : eqtype -> #f: cmp a -> place: nat{place > 0} -> 
                    s: seq a {Seq.length s > 0/\ Seq.sorted f s /\ Seq.length s > place} ->
                    Tot(unit)

let lemma_temp #a #f place s = 
    if (place > 4) then () else
    let element = Seq.index s place in 
    let s = fst (Seq.split s place) in 
    assert (Seq.length s > 0);
    assert (Seq.mem element s = false);

    () (*)
    let s = ordered_addition_l #a #f element s 
    in assert(Seq.sorted f s)



(*)
val lemma_left_split_sorted: #a: eqtype -> #f: cmp a -> place: nat{place  < 5} ->  
                            s: seq a {Seq.sorted f s /\ Seq.length s > place} ->
...
val lemma_temp_1: #a: eqtype -> #f : cmp a -> place: nat -> 
                Lemma(ensures)

val lemma_temp: #a : eqtype -> #f: cmp a -> place: nat{place > 1} ->     
                    s: seq a {Seq.sorted f s /\ Seq.length s > place} ->
                    Lemma(requires(Seq.sorted f (fst (Seq.split s (place-1)))))
                    (ensures (Seq.sorted f (fst (Seq.split s place))))
...
let rec lemma_left_split_sorted #a #f place s = 
    if (place < 5 ) then () 
    else lemma_left_split_sorted #a #f (place-1) s                        
