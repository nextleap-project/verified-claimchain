...
assume val current_place_gen: levels : nat -> sl: skipList a f -> Tot(list 'a)


private val sl_search: sl: skipList a f -> counter :nat{ length sl > counter}  -> required_level: nat -> ML(nat)
let rec sl_search sl counter  required_level = 
...
private val list_search : level: nat -> lst: list(nat) -> counter : nat -> ML list(nat)

let rec list_search level lst counter place=
...
let update_indexes sl place level counter sequence_regenerated =
	if counter = sl.length then sequence_regenerated else (
